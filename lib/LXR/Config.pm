...
sub _initialize {
	...
	my ($dir,  $arg);
	my ($host_pattern, $scheme, $hname, $port);
	my $routing;
	(my $caller) = $0 =~ m/([^\/]*)$/;

	unless ($host) {
		$host = $ENV{'REQUEST_SCHEME'};
		$host .= ':' if $ENV{'REQUEST_SCHEME'};
		$host .= '//' . $ENV{'HTTP_HOST'}
				. ':' . $ENV{'SERVER_PORT'};
# 		$host =~ s/:80$//;
	}
	...
	$host =~ s!(https?://$host_pattern(:\d+)?).*!$1!;	# sanitize scheme, host name and port
	$host =~ s!^(http://$host_pattern)$!$1:80!;
	$host =~ s!^(https://$host_pattern)$!$1:443!;
	($scheme, $hname, $port)
		= ($host =~ m!(https?:)(//$host_pattern)(:\d+)?!);
	$hname = lc($hname);
...
#	The target id ends up in 'baseurl' in both cases.
		# To give maximum flexibility in web server configuration (outside LXR)
		# check scheme and port only if lxr.conf 'host_names' specify them.
		# This can be part of (weak) "security" measures?
		# Also we can have the same tree on different ports for simultaneous
		# web servers (e.g. Apache and Nginx or others).
...
CANDIDATE: foreach my $config (@config[1..$#config]) {
		...
		if (scalar(@hostnames)>0) {
			foreach my $rt (@hostnames) {
				my ($rts, $rth, $rtp);	# scheme, host, port
				($rts, $rth)
					= ($rt =~ m!(https?:)?(//$host_pattern)!);		# ignore trailing /
				$rtp = undef;
				if ($rt =~ m!(:\d+)$!) {
					$rtp = $1;
				}
				if	(	$hname eq lc($rth)
					&&	(! defined($rts) || ($scheme eq $rts))
					&&	(! defined($rtp) || ($port eq $rtp))
		...
		# This is the legacy identification
		# Its is DEPRECATED and should not be used any more.
		# Match is not guaranteed in all circumstances.
			my @aliases;
			...
			foreach my $rt (@aliases) {
				...
				if	(	$hname eq lc($rth)
					...
					$config->{'baseurl'} = $rt;
        # BUG: fix for port not ported here because 'baseurl_aliases' are deprecated
        #       Consequently, expect access errors if used.
					%$self = (%$self, %$config);
					$$self{'parmgroupnr'} = $parmgroup;
					last CANDIDATE;
				}
...
#	Final checks on the parsing dispatcher
	...
	_ensuredirexists($self->{'tmpdir'}, $caller);
...
#	only when it is potentially used
		if (exists $self->{'glimpsebin'} && exists $self->{'swishbin'}) {
			...
			_ensuredirexists($self->{'glimpsedir'}, $caller);
			...
			_ensuredirexists($self->{'swishdir'}, $caller);
		} else {
...
=head2 C<_ensuredirexists ($chkdir, $caller)>
...
a I<string> containing the executing script name
...
sub _ensuredirexists {
	...
	my $caller = shift;
	if (! -d $chkdir) {
        if ('genxref' eq $caller) {
            make_path ($chkdir)
            or die "Couldn't make the directory $chkdir: ?!";
        } else {
            die "Directory $chkdir doesn't exist: ?!";
        }
	}
}
