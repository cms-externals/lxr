...
sub warning {
    ...
    if  (   0 > index($ENV{'SERVER_SOFTWARE'}, 'thttpd')
        &&  !$HTTP_inited
        ) {
    # Avoid sending diagnostics at early stage to thttpd
    # because a newline is sent even before we enter this sub
    # and is interpreted as end of HTTP-headers
...
sub httpinit {
	...
	# Parse and split URL
	my $path_info = http_wash($ENV{'PATH_INFO'});
	$path_info = clean_path($path_info);
	$path_info = '/' if $path_info eq '';
	$HTTP->{'path_info'} = $path_info;
	($HTTP->{'path_root'})
		= $path_info =~ m!^/([^/]+)!;

## Debugging dump of all environment variables
# for my $ev (sort keys %ENV){
# warn("\n$ev: $ENV{$ev}\n");
# }

	my $host_access;
	# LXR may be proxied. However, we need original information to be able
	# to compute relevant links. This is possible only if the proxy sends us
	# a carefully filled-in Forwarded HTTP header. Common X-Forwarded-For is
	# not enough because it can't pass the original port.
	# Without the Forwarded: HTTP header, don't try to run LXR behind a proxy.
	# In particular, thttpd can't be proxied because it discards all unknown
	# (from it, in its understanding of HTTP/1.0) headers.
	#
	# If you're proxying through Nginx, a workaround is possible by modifying
	# the response with configuration directives.
	if ($ENV{'HTTP_FORWARDED'}) {
		my $forwarded = $ENV{'HTTP_FORWARDED'};
		($host_access) = ($forwarded =~ m/proto\s*=\s*(\w+)/);
		$host_access .= '://';
		$host_access .= ($forwarded =~ m/host\s*=\s*((?:\[[^\]]+\]|[^:]+):\d+)/);
	} else {
		$host_access = $ENV{'REQUEST_SCHEME'};
		$host_access .= ':' if $ENV{'REQUEST_SCHEME'};
		$host_access .= '//' . $ENV{'HTTP_HOST'};
		$host_access .= ':' . $ENV{'SERVER_PORT'}
			if	(	(	$ENV{'SERVER_PORT'} ne '80'
					&&	(    $ENV{'REQUEST_SCHEME'} eq 'http'
						||  !defined($ENV{'REQUEST_SCHEME'})
						)
					)
				||	(	$ENV{'SERVER_PORT'} ne '443'
					&&	$ENV{'REQUEST_SCHEME'} eq 'https'
					)
				);
    }
    $HTTP->{'host_access'} = $host_access;
	my $script_path = $ENV{'SCRIPT_NAME'};
# die "server $ENV{'SERVER_SOFTWARE'} - script $ENV{'SCRIPT_NAME'} - path $ENV{'PATH_INFO'}\n";
# Fix bug in thttpd: if URI ends with a /, thttpd gets confused and returns SCRIPT_NAME
#       equal to what would be returned by REQUEST_URI if it existed.
	if (0 <= index($ENV{'SERVER_SOFTWARE'}, 'thttpd')) {
		$script_path =~ s!$path_info/$!!;
